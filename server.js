...
// n8n enviará esta key en el header X-Api-Key
...
// ─── HEALTH CHECK (sin auth, para verificar que está vivo) ──────────────────
...
/**
 * GET /citas
 * Listar citas. Parámetros opcionales:
 *   ?fecha=2026-03-10           → citas de ese día
 *   ?desde=2026-03-01&hasta=2026-03-31 → rango
 *   ?estado=confirmada          → por estado
 *   ?paciente=Ana               → búsqueda por nombre
 */
app.get('/citas', auth, async (req, res) => {
  try {
    ...
    if (paciente) {
      query += ` AND paciente_nombre ILIKE $${i++}`;
 ...
 * POST /citas
 * Crear nueva cita
 * Body: { paciente_nombre, paciente_telefono, paciente_email, tipo, doctor, fecha, hora, notas }
 */
app.post('/citas', auth, async (req, res) => {
  try {
    const {
      ...
      duracion_min = 30,
      notas = null,
      canal = 'manual', // whatsapp | telegram | web | email | manual
    } = req.body;
    ...
    // Verificar disponibilidad (misma fecha+hora+doctor)
    const conflict = await pool.query(
      `SELECT id FROM citas 
       WHERE doctor = $1 AND fecha = $2 AND hora = $3 AND estado != 'cancelada'`,
      [doctor, fecha, hora]
    );
    if (conflict.rows.length > 0) {
      return res.status(409).json({ 
        error: 'Horario no disponible',
        cita_id: conflict.rows[0].id 
      });
    }

    const result = await pool.query(
      `INSERT INTO citas 
        (paciente_nombre, paciente_telefono, paciente_email, tipo, doctor, fecha, hora, duracion_min, notas, canal, estado)
...
/**
 * PATCH /citas/:id
 * Actualizar cita (editar o reagendar)
...
/**
 * GET /disponibilidad
 * Consultar horarios libres para una fecha y doctor
 * ?fecha=2026-03-10&doctor=Dr.+García&duracion=30
 */
  ...
  try {
    ...
    const ocupadas = await pool.query(
      `SELECT hora FROM citas 
       WHERE fecha = $1 AND doctor = $2 AND estado != 'cancelada'`,
      [fecha, doctor]
    );
    ...
    res.json({ fecha, doctor, disponibles: slots, ocupadas: horasOcupadas });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
